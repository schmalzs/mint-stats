...
const removeTransfersToShared = data => data.filter(item => item.Category.toUpperCase() !== 'TRANSFER TO SHARED');
...
export default data => {
    return removeInvestmentTransfers(
        removeInvestments(
            removeCreditCardPayments(
                removeTransfers(
                    removeTransactionsWithAmountOfZero(
                        removeTransfersToShared(data)
                    )
                )
            )
        )
